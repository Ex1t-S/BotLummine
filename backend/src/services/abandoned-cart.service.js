...
async function resolveStoreCredentials() {
  const installation = await prisma.storeInstallation.findFirst({ orderBy: { installedAt: 'desc' } });
  ...
  if (!storeId || !accessToken) {
    throw new Error('Faltan credenciales de Tiendanube. Necesitás StoreInstallation o TIENDANUBE_STORE_ID y TIENDANUBE_ACCESS_TOKEN.');
  }
  return { storeId: String(storeId), accessToken };
}

const TIENDANUBE_API_VERSION = process.env.TIENDANUBE_API_VERSION || '2025-03';
const CHECKOUTS_PER_PAGE = Math.min(200, Math.max(20, Number(process.env.TIENDANUBE_ABANDONED_SYNC_PER_PAGE || 200)));
const FETCH_CONCURRENCY = Math.min(6, Math.max(1, Number(process.env.TIENDANUBE_ABANDONED_SYNC_CONCURRENCY || 3)));
...
function cleanString(value = '') { const normalized = String(value ?? '').trim(); return normalized || null; }
...
function parseDateOrNull(value) { if (!value) return null; const date = new Date(value); return Number.isNaN(date.getTime()) ? null : date; }
function toDecimalOrNull(value) { if (value === null || value === undefined || value === '') return null; return String(value); }
function mapAddress(cart) { return [cart?.shipping_address, cart?.shipping_number, cart?.shipping_floor, cart?.shipping_locality].filter(Boolean).join(' ').trim(); }
function chunkArray(values = [], size = 50) { const chunks = []; for (let i=0;i<values.length;i+=size) chunks.push(values.slice(i, i+size)); return chunks; }
function buildConcurrentPageList(pageStart, maxPage, concurrency) { const pages=[]; for(let o=0;o<concurrency;o+=1){const next=pageStart+o;if(next>maxPage) break; pages.push(next);} return pages; }

async function fetchCheckoutsPage({ storeId, accessToken, page }) {
  const params = new URLSearchParams({ page: String(page), per_page: String(CHECKOUTS_PER_PAGE) });
  ...
  for (let attempt = 1; attempt <= FETCH_RETRIES; attempt += 1) {
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404 && text.includes('Last page is')) return [];
...
function buildCartPayload(cart, storeId) {
  const products = Array.isArray(cart?.products) ? cart.products.map((product) => ({
    id: product?.id ?? null,
...
export async function syncAbandonedCarts(daysBack = DEFAULT_DAYS_BACK) {
  ...
  const cutoff = new Date(startedAt); cutoff.setDate(cutoff.getDate() - normalizedDaysBack);
  let pagesFetched = 0; let totalReceived = 0; let syncedCount = 0; let skippedOldCount = 0; let stopSync = false; const seenCheckoutIds = new Set();
  for (let pageStart = 1; pageStart <= MAX_PAGES && !stopSync; pageStart += FETCH_CONCURRENCY) {
    ...
    const pageResults = await Promise.all(pages.map((page) => fetchCheckoutsPage({ storeId, accessToken, page })));
    const validCarts = [];
    for (const carts of pageResults) {
      pagesFetched += 1;
      totalReceived += carts.length;
      if (!carts.length) { stopSync = true; continue; }
      for (const cart of carts) {
        const createdAt = parseDateOrNull(cart?.created_at);
        if (createdAt && createdAt < cutoff) { skippedOldCount += 1; continue; }
        validCarts.push(cart); seenCheckoutIds.add(String(cart.id));
      }
    }
    if (validCarts.length) syncedCount += await replaceCartBatch(validCarts, storeId);
    if (pageResults.every((items) => items.length < CHECKOUTS_PER_PAGE)) stopSync = true;
  }
  const staleWhere = { storeId, OR: [{ checkoutCreatedAt: { lt: cutoff } }, seenCheckoutIds.size ? { checkoutId: { notIn: Array.from(seenCheckoutIds) } } : null].filter(Boolean) };
  const removed = await prisma.abandonedCart.deleteMany({ where: staleWhere });
  return { ok: true, daysBack: normalizedDaysBack, pagesFetched, totalReceived, syncedCount, skippedOldCount, removedCount: removed.count, message: `Sync lista · páginas ${pagesFetched} · checkouts leídos ${totalReceived} · carritos guardados ${syncedCount} · limpiados ${removed.count}.` };
}
