...
export async function postSyncCustomers(req, res) {
	try {
		...
		const mode = normalizeSearch(req.body?.mode || req.query?.mode || 'auto') || 'auto';
		const pageBudget = toPositiveInt(req.body?.pageBudget || req.query?.pageBudget, undefined, {
			min: 1,
			max: 1000,
		});
		const result = await syncCustomers({ q, dateFrom, dateTo, mode, pageBudget });
...
export async function postFullSyncCustomers(req, res) {
	try {
		...
		const pageBudget = toPositiveInt(req.body?.pageBudget || req.query?.pageBudget, 250, {
			min: 1,
			max: 1000,
		});
		const result = await syncCustomers({ mode: 'full', pageBudget });
		return res.json(result);
	} catch (error) {
		console.error('[CUSTOMERS FULL SYNC ERROR]', error);

		const message = error?.message || 'Error sincronizando histórico de pedidos';
		const status = message.includes('Ya hay una sincronización de clientes en curso') ? 409 : 500;
