...
- The web service also starts a lightweight internal campaign dispatcher every minute by default. This keeps scheduled campaigns moving if the external cron service is missing or delayed. Set `CAMPAIGN_DISPATCHER_ENABLED=false` to disable it, or `CAMPAIGN_DISPATCHER_INTERVAL_MS` to tune the interval.
