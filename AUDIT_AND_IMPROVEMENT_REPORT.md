...
- Título: la revisión de pagos no conservaba una decisión auditable ni protegía reintentos
- Área: Backend/Frontend/Pagos/Multitenancy
- Ambiente: local
- Severidad: High
- Evidencia: el único contrato previo era el cambio genérico de cola; no existía entidad de auditoría, motivo, actor ni clave idempotente. Un timeout después de una escritura podía inducir al operador a repetir una acción sin forma de correlacionarla.
- Impacto: no era posible reconstruir quién finalizó una revisión ni distinguir aprobar, rechazar, pedir otro comprobante o derivar; tampoco había una frontera explícita de workspace para consultar ese historial.
- Causa: workflow de pagos modelado como estado visual de Inbox y no como operación de dominio.
- Solución: agregar `PaymentReviewAction` con actor, cola previa/final, motivo e idempotencia por workspace; endpoints GET/POST protegidos y acotados por `conversationId + workspaceId`; transacción que desactiva IA y deriva a HUMAN; conectar “Finalizar revisión y derivar” a `HANDOFF` reutilizando la misma clave tras un error de red.
- Estado: implementado y validado localmente para persistencia/API y acción HANDOFF. La UI de APPROVE/REJECT/REQUEST_NEW_PROOF y su diálogo de motivo siguen en P1.
...
- El endpoint de pagos requiere `PaymentReviewAction`; desplegar la migración antes de habilitar la ruta. Ante rollback de aplicación, conservar tabla y enum para evitar una operación destructiva innecesaria.
