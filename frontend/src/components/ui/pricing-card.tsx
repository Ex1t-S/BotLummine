...
function PlanName({ className, ...props }: React.ComponentProps<'div'>) {
	return <div className={cn("text-muted-foreground flex items-center gap-2 text-sm font-medium [&_svg:not([class*='size-'])]:size-4", className)} {...props} />;
}

function Badge({ className, ...props }: React.ComponentProps<'span'>) {
	return <span className={cn('border-foreground/20 text-foreground/80 rounded-full border px-2 py-0.5 text-xs', className)} {...props} />;
}
...
function Separator({ children = 'Upgrade to access', className, ...props }: React.ComponentProps<'div'> & { children?: string; className?: string }) {
	return (
